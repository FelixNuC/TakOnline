...
function App() {
  ...
  const loadGame = async () => {
    ...
    await withAsync(async () => {
      try {
        const existing = await request(`/api/games/room/${roomCode}`)
        setGame(existing)
      } catch {
        const created = await request(`/api/games/from-room/${roomCode}`, { method: 'POST' })
        setGame(created)
      }
    })
  }
  ...
  const copyRoomCode = async () => {
    ...
    try {
      ...
      setCopyMessage(`Copia manualmente: ${roomCode}`)
    }
    ...
    <main className={`app ${invertColors ? 'inverted' : ''}`}>
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          {room && (
            <button className="close-button" type="button" onClick={closeRoomView} aria-label="Salir de la room">
              ×
            </button>
          )}
        </div>
